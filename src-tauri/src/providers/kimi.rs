...
impl KimiProvider {
    ...
    fn load_api_key() -> Result<Option<String>, String> {
        ...
        crate::credentials::load_kimi_api_key(&config.credentials.kimi_backend)
    }
...
async fn query_kimi(api_key: &str) -> ServiceQuota {
    let config = crate::config::load_config();
    let builder = crate::proxy::apply_proxy(reqwest::Client::builder(), "Kimi", &config.proxy.kimi);
    let client = builder.build().unwrap_or_else(|e| {
        log::warn!("Failed to build Kimi HTTP client, falling back to default client: {e}");
        reqwest::Client::new()
    });
